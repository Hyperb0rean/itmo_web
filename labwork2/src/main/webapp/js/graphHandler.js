...
const AXES_COLOR = '#494949';
const CIRCLE_COLOR = '#FFD700FF';
const TRIANGLE_COLOR = '#FFD700FF';
const RECTANGLE_COLOR = '#FFD700FF';
...
drawArea = (r) => {
    const circlePath = 'M ' + (convertX(-r)) + ', ' + (convertY(0)) + ' ' +
        ...
        '90 0,1 ' + (convertX(0)) + ', ' + (convertY(r)) + ' L 150,150 Z'
...
drawPoint = (x, y, result, pointScale) => {
    let color = (result === "false" || result === undefined) ? '#ff0000' : '#ff0000';
...
getCoords = (event) => {
    let coordinates = {};
    coordinates.x = (event.pageX-822)/26;
    coordinates.y = (-event.pageY+299)/26;
